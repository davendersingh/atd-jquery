...
AtDCore.prototype.findSuggestionFromCache = function(element) {
    var text = element.innerHTML;
    var context = ( this.getAttrib(element, 'pre') + "" ).replace(/[\\,!\\?\\."\s]/g, '');
    if (this.getAttrib(element, 'pre') == undefined)
    {
    ...
    var errorDescription = undefined;
    var cachedObjKey = undefined;
    $.each( results_cache, function( k, element ){
        var len = element.suggestions.length;
        cachedObjKey = k;
        for (var i = 0; i < len; i++) {
            var key = element.suggestions[i]["string"];
        ...
        if(errorDescription != undefined){
            AtD.count = results_cache[cachedObjKey].count;
            AtD.suggestions = errorDescription;
            AtD.counter = AtD.count;
            AtD.container = cachedObjKey;
            return errorDescription;
        }
...
{
    rpc : '',
    rpc_css : 'http://69.164.202.77/proxycss.php?data=',
    rpc_css_lang : 'en',
    api_key : 'csssdsproxy',
    i18n : {},
    listener : {}
...
AtD.processXML = function(container_id, responseXML) {
    ...
    results_cache[container_id] = results;
    if (results.count > 0)
...
AtD.suggest = function(element) {
    ...
    if(errorDescription == undefined)
        errorDescription = AtD.core.findSuggestionFromCache(element);
